...
\begin{center}
 \begin{extern}%style="width:25rem" alt="Arbre pondéré"
    ...
    \node[noeud] (ra) at ({2*\width},{-1*\height}) {$B$};
     \draw (r) -- (ra) node [poids] {$\dfrac{1}{3}$};
    \node[noeud] (raa) at ({3*\width},{0*\height}) {$B$};
     \draw (ra) -- (raa) node [poids] {$\dfrac{n-1}{3n-1}$};
    \node[noeud] (rab) at ({3*\width},{-1*\height}) {$B$};
     \draw (ra) -- (rab) node [poids] {$\dfrac{n-2}{3n-1}$};
    \node[noeud] (rac) at ({3*\width},{-2*\height}) {$F$};
     \draw (ra) -- (rac) node [poids] {$0,9$};
    \node[noeud] (rb) at ({2*\width},{-3.5*\height}) {$\overline{A}$};
     \draw (r) -- (rb) node [poids] {$0,4$};
    \node[noeud] (rba) at ({3*\width},{-3*\height}) {$V$};
     \draw (rb) -- (rba) node [poids] {$0,8$};
    \node[noeud] (rbb) at ({3*\width},{-4*\height}) {$F$};
