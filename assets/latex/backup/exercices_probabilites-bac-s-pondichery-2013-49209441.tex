...
\begin{center}
 \begin{extern}%style="width:25rem" alt="Arbre pondéré"
    \resizebox{11cm}{!}{
       ...
       \tikzstyle{poids}=[fill=white,font=\footnotesize,midway]
    \node[noeud] (r) at ({1*\width},{-1.5*\height}) {$$};
    \node[noeud] (ra) at ({2*\width},{-0.5*\height}) {$E_2$};
     \draw (r) -- (ra) node [poids] {$0,04$};
    \node[noeud] (raa) at ({3*\width},{0*\height}) {$E_3$};
     \draw (ra) -- (raa) node [poids] {$0,24$};
    \node[noeud] (rab) at ({3*\width},{-1*\height}) {$\overline{E_3}$};
     \draw (ra) -- (rab) node [poids] {$0,76$};
    \node[noeud] (rb) at ({2*\width},{-2.5*\height}) {$\overline{E_2}$};
     \draw (r) -- (rb) node [poids] {$0,96$};
    \node[noeud] (rba) at ({3*\width},{-2*\height}) {$E_3$};
     \draw (rb) -- (rba) node [poids] {$0,04$};
    \node[noeud] (rbb) at ({3*\width},{-3*\height}) {$\overline{E_3}$};
     \draw (rb) -- (rbb) node [poids] {$0,96$};
       \end{tikzpicture}
      }
   \end{extern}
\end{center}
...
% >E_n:p_n
% >>E_{n+1}:0,24
% >>\overline{E_{n+1}}:0,76
% >\overline{E_n}:1-p_n
% >>E_{n+1}:0,04
% >>\overline{E_{n+1}}:0,96
%--
\begin{center}
 \begin{extern}%style="width:25rem" alt="Arbre pondéré"
    \resizebox{11cm}{!}{
