...
\begin{center}
 \begin{extern}%style="width:25rem" alt="Arbre pondéré"
    ...
    \node[noeud] (ra) at ({2*\width},{-0.5*\height}) {$A$};
     \draw (r) -- (ra) node [poids] {$fr$};
    \node[noeud] (raa) at ({3*\width},{0*\height}) {$V$};
     \draw (ra) -- (raa) node [poids] {$0,1$};
    \node[noeud] (rab) at ({3*\width},{-1*\height}) {$F$};
     \draw (ra) -- (rab) node [poids] {$0,9$};
    \node[noeud] (rb) at ({2*\width},{-2.5*\height}) {$\overline{A}$};
     \draw (r) -- (rb) node [poids] {$0,4$};
    \node[noeud] (rba) at ({3*\width},{-2*\height}) {$V$};
     \draw (rb) -- (rba) node [poids] {$0,8$};
    \node[noeud] (rbb) at ({3*\width},{-3*\height}) {$F$};
